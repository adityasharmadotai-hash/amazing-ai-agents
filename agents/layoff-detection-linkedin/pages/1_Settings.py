"""Settings page — every API key in one place, with step-by-step instructions
for generating each one, plus a live status check.

Two ways to configure the app:
  • **Streamlit Cloud**: add keys under App → Settings → Secrets (TOML). These
    are read automatically at startup.
  • **This page**: paste keys to set them for the current session (handy for
    local runs and quick testing). Session values override secrets until the
    app restarts.
"""
...
st.set_page_config(page_title="Settings · LayoffScout AI",
                   page_icon="⚙️", layout="wide")
st_common.bootstrap_env()
st_common.inject_css()

from agent import config  # noqa: E402  (after bootstrap)

st.title("⚙️ Settings & API keys")
st.caption("Configure every service the agent uses. Each field explains exactly "
           "how to generate the key. Nothing here is sent anywhere except the "
           "service it belongs to.")

# ── Step 1: choose the LinkedIn data source (outside the form so switching it
#    immediately reveals only the relevant fields) ──────────────────────────
...
if source == "serpapi":
    st.info("**SerpAPI mode:** you only need a SerpAPI key below — Apify is not "
            "used at all. Great for getting started for free.", icon="🟢")
else:
...
if missing:
    st.warning("Missing required keys: **" + ", ".join(missing) + "**", icon="⚠️")
else:
...
with st.expander("ℹ️ How configuration works"):
    st.markdown(
        ...
        "paste keys in TOML form (see the template at the bottom of this page). "
        "They load automatically every time the app starts.\n"
        "- **Locally**, copy `.env.example` to `.env` and fill it in, *or* paste "
        "keys below to set them just for this session.\n"
...
st.caption("Copy this into your app's **Settings → Secrets** box and fill in the "
           "values. It already reflects your chosen source; delete any optional "
           "lines you don't use.")
_DEFAULTS = {
...
for f in st_common.CONFIG_KEYS:
    # skip the other backend's keys in the template too
    if f.get("backend", source) != source:
        continue
    template_lines.append(f'{f["key"]} = "{_DEFAULTS.get(f["key"], "")}"')
